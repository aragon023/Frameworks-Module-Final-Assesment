    ...
    <Offcanvas show={show} onHide={onHide} scroll backdrop className="bg-light" style={{ width: 280 }}>
      ...
      <Offcanvas.Body>
        ...
        <Nav className="flex-column gap-2 mb-4">
          ...
          <Nav.Link href="/pets" className="btn btn-light text-start">🐾 Pets</Nav.Link>
        ...
        {isLoading ? (
          ...
          <div className="text-danger small mb-3">Failed to load members.</div>
        ) : (
          <ListGroup variant="flush" className="mb-4">
            {(data ?? []).map((m) => (
              <ListGroup.Item key={m.id} className="d-flex align-items-center gap-2">
                {m.avatar_url ? (
            ...
            {(!data || data.length === 0) && (
              <ListGroup.Item className="text-muted">No members yet</ListGroup.Item>
            )}
          </ListGroup>
        )}

        <Button variant="success" className="w-100">Invite Member</Button>
      </Offcanvas.Body>
    </Offcanvas>
  );
}
